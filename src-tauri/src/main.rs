...
fn build_menu(app: &tauri::AppHandle) -> tauri::Result<tauri::menu::Menu<tauri::Wry>> {
    let show_status_bar = get_saved_show_status_bar(app);
    let lang = get_saved_language(app);
    let l = lang.as_str();
    ...
    // ── SSH menu ──────────────────────────────────────────────────
    let ssh_profiles = shell_profile::load_ssh_profiles(app);
    let ssh_config_entries = ssh_config::read_ssh_config().unwrap_or_default();
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Rebuild the macOS menu bar whenever shell profiles are added or removed.
            let rebuild_handle = app.app_handle().clone();
            app.app_handle().listen("vibe99:profiles-changed", move |_| {
                if let Ok(menu) = build_menu(&rebuild_handle) {
